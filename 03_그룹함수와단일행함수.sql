-- 그룹함수와 단일행함수
-- 단일행 함수 (SINGLE POW FUNCTION) : N개의 값을 넣어 N개의 결과를 리턴
...
-- LPAD / RPAD : 주어진 문자열에 임의의 문자열을 덧붙여 길이 N의 문자열을 반환하는 함수
SELECT
        LPAD(EMAIL, 20, '#')
    FROM EMPLOYEE;
...
-- 사원명, 생년, 생월, 생일을 각각 분-- EMPLOYEE 테이블에서 직원의 주민번호를 조회하여
...
SELECT
    ...
    FROM EMPLOYEE;
-- LOWER / UPPER / INTCAP : 대소문자 변경해주는 함수
SELECT LOWER('Welcome To My World') FROM DUAL;
SELECT UPPER('Welcome To My World') FROM DUAL;
SELECT INITCAP('Welcome To My World') FROM DUAL;
--CONCAT : 문자열 두 개를 하나의 문자열로 합친 후 리턴
SELECT CONCAT('가나다라', 'ABCD') FROM DUAL;
SELECT '가나다라' || 'ABCD' FROM DUAL;리하여 조회
-- 단, 컬럼의 별칭은 사원명, 생년, 생월, 생일로 한다.
...
SELECT SYSDATE, NEXT_DAT(SYSDATE, '금요일') FROM DUAL;
SELECT SYSDATE, NEXT_DAT(SYSDATE, 6) FROM DUAL;
...
-- 형변환 함수
-- TO_CHAR(날짜, [포멧]) : 날짜형 데이터를 문자형 데이터로 전송
...
-- YY와 RR의 차이
-- RR은 두 자리 년도를 네 자리로 바꿀 때
-- 바꿀 년도가 50년 미만인 경우 2000년대를 적용,
-- 50년 이상이면 1900년대를 적용한다.
  
SELECT
        TO_CHAR(TP_DATE('980630', 'YYMMDD'), 'YYYY-MM-DD')
...
-- 문자는 날짜로 자동 형변호나 되지만, 숫자는 날짜로 자동 형변환 안됨
