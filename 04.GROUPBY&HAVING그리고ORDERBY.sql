...
/*
5 : SELECT 컬럼면 AS 별칭, 계산식, 함수식
1 : FROM 참조할 테이블 명
2 : WHERE 행을 식별하기 위한 조건식
3 : GROUP BY 그룹을 묶을 컬럼명
4 : HAVING 그룹을 식별하기 위한 조건식
6 : ORDER BY 컬럼명 | 별칭 | 컬럼 순번 정렬방식([ASC]|DESC) [NULLS FIRST | LAST]
*/
SELECT COUNT(*) 사원수
     , DEPT_CODE
  FROM EMPLOYEE
...
SELECT DEPT_CODE   부서코드
 ...
 -- 직원 테이블에서 보너스를 받는 직원의 직급코드, 사원 수를 조회하여
 -- 직급코드 순으로 오름차순 정렬하세요
...
-- 집계함수
-- ROLLUP 함수 : 그룹별로 주간 집계 처리를 하는 함수로 GROUP BY 절에서만 사용한다.
SELECT
        JOB_CODE
      , SUM(SALARY)
    FROM EMPLOYEE
